...
class Generator(torch.nn.Module):
    def __init__(self, resize, in_channels, hidden_size, num_classes):
        ...
        self.features = torch.nn.Sequential(
            ...
            torch.nn.LeakyReLU(0.2),
            torch.nn.ConvTranspose2d(hidden_size * 2, hidden_size * 2 if resize == 64 else hidden_size, 4, 2, 1, bias=False),
            torch.nn.BatchNorm2d(hidden_size * 2 if resize == 64 else hidden_size),
            torch.nn.LeakyReLU(0.2),
            torch.nn.ConvTranspose2d(hidden_size * 2, hidden_size, 4, 2, 1, bias=False) if resize == 64 else torch.nn.Identity(),
            torch.nn.BatchNorm2d(hidden_size) if resize == 64 else torch.nn.Identity(),
            torch.nn.LeakyReLU(0.2) if resize == 64 else torch.nn.Identity(),
...
class Discriminator(torch.nn.Module):
    def __init__(self, resize, in_channels, hidden_size, num_classes):
        ...
        self.features = torch.nn.Sequential(
            ...
            torch.nn.LeakyReLU(0.2),
            torch.nn.Dropout(0.5),
            torch.nn.Conv2d(hidden_size * 4, hidden_size * 4, 4, 2, 1, bias=False) if resize == 64 else torch.nn.Identity(),
            torch.nn.BatchNorm2d(hidden_size * 4) if resize == 64 else torch.nn.Identity(),
            torch.nn.LeakyReLU(0.2) if resize == 64 else torch.nn.Identity(),
            torch.nn.Dropout(0.5) if resize == 64 else torch.nn.Identity(),
