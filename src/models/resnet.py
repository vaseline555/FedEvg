...
class ResNet(torch.nn.Module):
    def __init__(self, config, block, resize, in_channels, hidden_size, num_classes, penult_spectral_norm):
        ...
        self.classifier = torch.nn.Sequential(
            ...
            torch.nn.ReLU(),
            torch.nn.Flatten(),
            torch.nn.utils.spectral_norm(torch.nn.Linear((resize // 8)**2 * (hidden_size * 8), num_classes, bias=True)) \
                if penult_spectral_norm \
                    else torch.nn.Linear((resize // 8)**2 * (hidden_size * 8), num_classes, bias=True)
        )
