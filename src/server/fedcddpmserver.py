...
from src.client.fedcddpmclient import GaussianDiffusionSampler
...
class FedcddpmServer(FedavgServer):
    ...
    def _log_results(self, resulting_sizes, results, eval, participated, save_raw):
        losses, losses_recon, metrics, num_samples = list(), list(), defaultdict(list), list()
        generated = list()
        ...
        for identifier, result in results.items():
            ...
            if eval: # get loss and metrics
                ...
                client_log_string += f'| loss (D): {loss_recon:.4f} '
                losses_recon.append(loss_recon)
        ...
        total_log_string += f'\n    - Loss: Avg. ({weighted:.4f}) Std. ({std:.4f}) | Loss (recon.): Avg. ({weighted_recon:.4f}) Std. ({std_recon:.4f})'
        result_dict['loss'] = {
    ...
    def _central_evaluate(self):
        ...
        noise = torch.rand(self.args.num_classes * self.args.spc, self.args.in_channels, self.args.resize, self.args.resize).to(self.args.device)
        inputs_synth = sampler(noise, targets_synth)
        sampler.to('cpu')
        ...
        self.writer.add_image(
            ...
            torchvision.utils.make_grid(inputs_synth.clip(-1., 1.).mul(0.5).add(0.5), nrow=self.args.spc), 
        ...
        np.savez(
            ...
            inputs=inputs_synth.clip(-1., 1.).mul(0.5).add(0.5).numpy(),
        ...
        for (synth, _), (inputs, targets) in zip(
            ...
            inputs, targets = inputs.clip(-1., 1.).mul(0.5).add(0.5).to(self.args.device), targets.to(self.args.device)
            outputs = self.classifier(inputs)
