...
def load_dataset(args):
    ...
    if args.use_model_tokenizer or args.use_pt_model:
        assert args.model_name in ['DistilBert', 'SqueezeBert', 'MobileBert', 'EfficientNetPT', 'Bert'], 'Please specify a proper model!'
    ...
    if args.use_model_tokenizer:
        ...
        tokenizer = getattr(module, f'{args.model_name}Tokenizer').from_pretrained(TOKENIZER_STRINGS[args.model_name], do_lower_case=True)
    ...
    if args.dataset in ['FEMNIST', 'Shakespeare', 'Sent140', 'CelebA', 'Reddit']: # 1) for a special dataset - LEAF benchmark...
        ...
        if args.dataset in ['FEMNIST', 'CelebA']:
            ...
            if (args.crop is None) or (args.resize is None):
                logger.info(f'[LOAD] Dataset `{args.dataset}` may require `crop` or `resize` argument; (recommended: `FEMNIST` - 28, `CelebA` - 84)!')
    ...
    elif args.dataset in torchvision.datasets.__dict__.keys(): # 2) for downloadable datasets in `torchvision.datasets`...
    ...
    #    raw_train, raw_test, args = fetch_torchtext_dataset(args=args, dataset_name=args.dataset, root=args.data_path, seq_len=args.seq_len, tokenizer=tokenizer, num_embeddings=args.num_embeddings) 
        ...
        _check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
        _check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
        split_map, client_datasets, args = fetch_speechcommands(args=args, root=args.data_path)
    ...
    elif args.dataset == 'Berka':
        _check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
        _check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
        split_map, client_datasets, args = fetch_berka(args=args, root=args.data_path)
    
    elif args.dataset == 'MQP':
        _check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
        _check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
        _check_and_raise_error(args.use_model_tokenizer, False, '(i.e., NO) use of pre-trained tokenizer', True) 
        split_map, client_datasets, args = fetch_mqp(args=args, root=args.data_path, tokenizer=tokenizer)

    elif args.dataset == 'Adult':
        _check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
        _check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
        split_map, client_datasets, args = fetch_adult(args=args, root=args.data_path)
    
    elif args.dataset == 'FFMNIST':
        _check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
        _check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
        split_map, client_datasets, args = fetch_ffmnist(args=args, root=args.data_path, transform=_get_transform(args, train=True))
    
    elif args.dataset == 'Cover':
        _check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
        _check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
        split_map, client_datasets, args = fetch_cover(args=args, root=args.data_path)  
    
    elif args.dataset == 'GLEAM':
        _check_and_raise_error(args.split_type, 'pre', 'split scenario', False)
        _check_and_raise_error(args.eval_type, 'local', 'evaluation type', False)
        split_map, client_datasets, args = fetch_gleam(args=args, root=args.data_path)
