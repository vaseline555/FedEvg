...
torch.nn.ECELoss = CalibrationError
    ...
    def aggregate(self, total_len, curr_step=None):
        ...
        if curr_step is not None:
            ...
            if len(running_figures.keys()) > 1:
                for key in running_figures.keys():
                    if key != 'loss':
                        self._results[curr_step][key] = running_figures[key]
            ...
            if len(running_figures.keys()) > 1:
                for key in running_figures.keys():
                    if key != 'loss':
                        self._results[key] = running_figures[key]
                self._results['generated'] = self.figures['generated']
        self.figures = defaultdict(int)
