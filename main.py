...
if __name__ == "__main__":
    ...
    parser.add_argument('--max_workers', help='number of threads', type=int, default=None)
    ...
    parser.add_argument('--model_name', help='a model to be used (NOTE: case sensitive)', type=str,
        choices=[
            'TwoNN', 'TwoCNN', 'SimpleCNN', 'FEMNISTCNN', 'Sent140LSTM', 'CelebACNN',
    ...
    parser.add_argument('--num_layers', help='number of layers in recurrent cells', type=int, default=None)
    parser.add_argument('--num_embeddings', help='size of an embedding layer (i.e., vocab size)', type=int, default=None)
    parser.add_argument('--embedding_size', help='output dimension of an embedding layer', type=int, default=None)
    ...
    parser.add_argument('--algorithm', help='federated learning algorithm to be used', type=str,
        choices=[
            ...
            'fedcgan', 'fedcvae', 'fedcddpm', 'fedevg', 'feddm', 'fedcvaekd'
        ], 
        required=True
    )
    ...
    parser.add_argument('--beta1', help='first server momentum factor (when positive, algorithm should be one of [`fedavgm`, `fedadam`, `fedyogi`, `fedadagrad`])', type=float, choices=[Range(0., 1.)], default=0)
    parser.add_argument('--beta2', help='second server momentum factor (valid only if the algorithm is [`fedadam`, `fedyogi`, `fedadagrad`])', type=float, choices=[Range(0., 1.)], default=0)
    
    # optimization arguments
    parser.add_argument('--no_shuffle', help='do not shuffle data when training (if passed)', action='store_true')
    parser.add_argument('--use_class_weights', help='use class-weighted loss adjustment proportional to class-wise sample ratio (if passed)', action='store_true')
    ...
    parser.add_argument('--betas', help='momentum factors for Adam optimizer', type=tuple, default=(0.5, 0.999))
    ...
    parser.add_argument('--lr_decay', help='decay rate of learning rate', type=float, choices=[Range(0., 1.)], default=1)
    parser.add_argument('--lr_decay_step', help='intervals of learning rate decay', type=int, default=1)
    parser.add_argument('--criterion', help='objective function (NOTE: should be a submodule of `torch.nn`, thus case-sensitive) (SPEICAL LOSS: `Seq2SeqLoss`, `DiceLoss`', type=str, required=True)
